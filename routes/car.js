...
// Uncomment ini kalau mau test JWT nya
// router.all('*', auth, async (req, res, next) => {
//   // console.log(req.user._email)
//   next();
// });

router.get('/all/model', async (req, res) => {
  const data = await TblModelPorsche.findAll({
    where: {
      status_del: 1
    },
    order: [
...
router.get('/all/model/search', async (req, res, next) => {
  ...
  // Input Validation
  const validation = validateModelSearch(req.query);
...
router.get('/all/model/type', async (req, res, next) => {
  ...
  // Input Validation
  const validation = validateTypeModelGetId(req.query);
  ...
  const data = await ViewTypeModelPorsche.findAll({
    where: {
      status_del: 1,
      id_model: id
    },
...
router.get('/all/model/type/search', async (req, res, next) => {
  ...
  // Input Validation
  const validation = validateTypeModelSearch(req.query);
...
router.get('/all/type/colour', async (req, res, next) => {
  ...
  // Input Validation
  const validation = validateTypeColourId(req.query);
  ...
  const data = await ViewTypeColour.findAll({
    where: {
      id_type: id,
      status_del: 1
    },
...
router.get('/one/colour/detail', async (req, res, next) => {
  ...
  // Input Validation
  const validation = validateTypeColourIdColour(req.query);
  ...
  const data = await ViewTypeColour.findOne({
    where: {
      id_type: id,
      id_colour: colour,
      status_del: 1
...
router.get('/banner/porsche', async (req, res, next) => {
  const data = await ViewBannerPorsche.findAll();
...
router.get('/currency', async (req, res, next) => {
  ...
  res.status(200).send({
    ...
    total: 1,
    data: {
      idr_currency: total.toLocaleString('id-ID', {
        style: 'currency',
        currency: 'IDR'
      })
    }
